#!/usr/bin/env python3
"""
Denim Tears -> Discord Alert Bot
------------------------------------
⚠️ UNCERTAIN — TEST BEFORE TRUSTING THIS ONE.
Denim Tears' storefront was built by an agency using Shopify Hydrogen
(a "headless" React-based framework), based on public case studies. Headless
Shopify storefronts sometimes disable the classic /products.json feed that
Lemkus and Nude Project rely on, since they fetch data through Shopify's
Storefront GraphQL API instead.

This script tries the classic JSON approach first since it's the simplest
path IF it works. Watch the very first deploy logs closely:
  - If you see "First run — baseline built with X product(s)" -> it worked,
    Denim Tears still exposes the feed despite being headless.
  - If you see repeated 404 or empty-JSON errors -> the feed is disabled,
    and this bot would need to be rebuilt using Shopify's GraphQL Storefront
    API instead (a different, slightly bigger build — just let me know).

Posts to Discord whenever it sees:
  1) A brand new product added to a watched collection
  2) A restock (a size/variant that was sold out becomes available again)

SETUP: same as the Lemkus bot.
  ...
  4. python denimtears_discord_bot.py
"""
...
BASE_URL = "https://denimtears.com"

# Collection handles to watch — confirmed real ones from denimtears.com/collections
COLLECTIONS = [
    "new-arrivals",
    "all",
    "hoodies",
...
STATE_FILE = "denimtears_state.json"
REQUEST_TIMEOUT = 15

HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; DenimTearsAlertBot/1.0; +personal use)"
}

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
log = logging.getLogger("denimtears-bot")
...
def build_embed(product, collection_handle, kind, all_available_sizes=None, changed_sizes=None):
    ...
    embed = {
        ...
        "footer": {"text": "Denim Tears Alert Bot"},
    }
    if image_url:
...
def main():
    ...
    log.info("Starting Denim Tears alert bot. Watching: %s", ", ".join(COLLECTIONS))

    post_to_discord({
        "title": "✅ Denim Tears Alert Bot is online",
