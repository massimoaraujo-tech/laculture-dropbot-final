#!/usr/bin/env python3
"""
Nude Project -> Discord Alert Bot
------------------------------------
Same approach as the Lemkus bot: Nude Project runs on a standard Shopify
Liquid theme (confirmed — their pages show raw Shopify Liquid errors, a good
technical sign it's NOT a headless/custom storefront), so the public
/products.json feed on each collection should work the same way.

⚠️ Counterfeit-site note: there are copycat domains impersonating Nude
Project (e.g. nudeproject-official.com, nudeprojectbrand.shop). The real
site, confirmed via their official Shopify presence, is nude-project.com —
that's what BASE_URL points to below. Don't repoint this at a look-alike
domain without double-checking it against their verified Instagram bio link.
...
SETUP: same as the Lemkus bot.
  ...
  4. python nudeproject_discord_bot.py
"""
...
# nude-project.com — the handle is the part after /collections/ in the URL.
COLLECTIONS = [
    "new-arrivals",
    "sale",
    "t-shirts",
...
STATE_FILE = "nudeproject_state.json"
REQUEST_TIMEOUT = 15

HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; NudeProjectAlertBot/1.0; +personal use)"
}

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
log = logging.getLogger("nudeproject-bot")
...
def build_embed(product, collection_handle, kind, all_available_sizes=None, changed_sizes=None):
    ...
    embed = {
        ...
        "footer": {"text": "Nude Project Alert Bot"},
    }
    if image_url:
...
def main():
    ...
    log.info("Starting Nude Project alert bot. Watching: %s", ", ".join(COLLECTIONS))

    post_to_discord({
        "title": "✅ Nude Project Alert Bot is online",
